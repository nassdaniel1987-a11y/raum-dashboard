<script lang="ts">
	...
	import type { DailyConfig, Room, RoomStatus, RoomWithConfig } from '$lib/types';
	...
	let {
		...
		nowOverride = null,
		roomsOverride = null,
		roomStatusesOverride = null,
		dailyConfigsOverride = null,
		viewWeekdayOverride = null,
		...
		roomStatusesOverride?: Map<string, RoomStatus> | null;
		dailyConfigsOverride?: Map<string, DailyConfig> | null;
		viewWeekdayOverride?: number | null;
		runnerNameOverride?: string | null;
		initialPageId?: string | null;
	}>();

	const AUTO_PAGE_SECONDS = 10;
	...
	const PAGE_DEFINITIONS = [
		...
		{ id: 'extern', label: 'Außenbereich', short: 'AUSSEN', floors: ['extern'] }
	];
	...
	let sourceRooms: Room[] = $derived(roomsOverride ?? $rooms);
	let sourceStatuses: Map<string, RoomStatus> = $derived(roomStatusesOverride ?? $roomStatuses);
	let sourceConfigs: Map<string, DailyConfig> = $derived(dailyConfigsOverride ?? $dailyConfigs);
	let sourceWeekday: number = $derived(viewWeekdayOverride ?? $viewWeekday);
	let sourceRunnerName: string = $derived(runnerNameOverride ?? $runnerNameStore);

	let allRooms: RoomWithConfig[] = $derived(
		sourceRooms
			.map((room: Room) => {
				const config = sourceConfigs.get(`${room.id}-${sourceWeekday}`) ?? null;
				const status = sourceStatuses.get(room.id) ?? null;
				return {
					...room,
					config,
					status,
	...
	let activePages = $derived((): Array<(typeof PAGE_DEFINITIONS)[number] & { rooms: RoomWithConfig[] }> => {
		const pages = PAGE_DEFINITIONS.map((page) => ({
			...page,
			rooms: allRooms.filter((room: RoomWithConfig) => page.floors.includes(room.floor))
	...
	let totalOpen = $derived(allRooms.filter((room: RoomWithConfig) => room.isOpen).length);
	...
	function roomState(room: RoomWithConfig): 'open' | 'closed' | 'opening' | 'closing' {
		const nowDate = nowOverride ?? $currentTime;
	...
	onMount(() => {
		if (initialPageId) {
			const index = activePages().findIndex((page) => page.id === initialPageId);
...
<div class="calm-shell" ontouchstart={handleTouchStart} ontouchend={handleTouchEnd}>
	{#if activePages().length === 0}
		<div class="empty-state">
			<div class="empty-label">Sandbox</div>
			<h2>Keine Raeume vorhanden</h2>
			<p>Diese Ansicht wartet auf Raumdaten oder Mock-Daten.</p>
		</div>
	{:else}
			...
			<div class="status-strip" aria-label="Statusuebersicht">
				...
				{#if sourceRunnerName}
					<div class="runner-pill">
						...
						<span class="runner-name">{sourceRunnerName}</span>
					</div>
				{/if}
			</div>
		</header>
		...
		<nav class="page-tabs" aria-label="Etagen">
			{#each activePages() as page, index (page.id)}
				...
				>
					...
					{#if index === pageIndex}
						{#key pageIndex}
							<span class="tab-progress" style="animation-duration: {AUTO_PAGE_SECONDS}s;"></span>
						{/key}
					{/if}
				</button>
			{/each}
